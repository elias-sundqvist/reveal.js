...
window.RevealMath = ()=> 
{
	...
	options.katexScript     = options.katexScript     || 'plugin/math-katex/lib/katex-0.12.0/katex.min.js';
	options.katexStylesheet = options.katexStylesheet || 'plugin/math-katex/lib/katex-0.12.0/katex.min.css';
	...
	var defaults = {
		...
		// found
		cdn: {
			script:     'https://cdnjs.cloudflare.com/ajax/libs/KaTeX/0.12.0/katex.min.js',
			stylesheet: 'https://cdnjs.cloudflare.com/ajax/libs/KaTeX/0.12.0/katex.min.css'
		},
	...
	var replaceFormulaTex = (function() {
		...
		return function ( mode, markup ) {
			...
			function replacer( _, lookbehind, group, offset ) {
				...
				var markup;
				try {
					markup = lookbehind + katex.renderToString( "\\newcommand{\\id}[2]{\\htmlClass{animatedKatex}{\\htmlData{id=#1}{#2}}}"+group, {strict: false, trust: true, throwOnError: false});
				}
				catch ( error ) {
	...
	function replaceFormulas() {
		...
		each( wrappedElements, function ( e ) {
			...
			try {
				e.innerHTML = katex.renderToString( "\\newcommand{\\id}[2]{\\htmlClass{animatedKatex}{\\htmlData{id=#1}{#2}}}"+formula, {strict: false, trust: true, throwOnError: false} );
